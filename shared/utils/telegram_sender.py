...
async def send_processed_image_to_telegram(user_id: int, image_bytes: bytes):
    ...
    await bot.send_document(
        chat_id=user_id, document=file, caption="Вот ваше обработанное изображение!"
    )


async def send_resize_options_to_telegram(
    user_id: int, image_bytes: bytes, width: int, height: int, resize_options: list
):
    ...
    for option in resize_options:
        callback_data = json.dumps(
            {"action": "resize", "width": option["width"], "height": option["height"]}
        )
        keyboard.append(
            [
